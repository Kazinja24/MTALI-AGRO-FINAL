...
export default function AdminBlogPanel() {
  ...
  const canCreate = useMemo(
    () => sanitizePlainText(titleEn).length > 0 && sanitizePlainText(bodyEn).length > 0,
    [titleEn, bodyEn],
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        toast.error(getErrorMessage(error, t("admin.loadPostsFailed")));
      } finally {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      const safeTitle = sanitizePlainText(titleEn);
      payload.append("title_en", safeTitle);
      payload.append("category", category);
      payload.append("excerpt_en", sanitizePlainText(excerptEn));
      payload.append("body_en", sanitizePlainText(bodyEn));
      ...
      toast.error(getErrorMessage(error, t("admin.createPostFailed")));
    }
  }
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      toast.error(getErrorMessage(error, t("admin.deletePostFailed")));
    }
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-5">
      <form
        ...
        className="lg:col-span-2 space-y-4 rounded-3xl border border-border bg-card p-6 h-fit"
      >
        ...
        <button
          disabled={!canCreate}
          className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-soft disabled:opacity-60"
          type="submit"
        >
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {p.category} · {p.published_at ? "Published" : "Unpublished"}
                    </p>
                  </div>
                  <button
                ...
                <p className="mt-3 line-clamp-3 text-sm text-foreground/80">
                  {truncatePlainText(p.body_en, 180)}
                </p>
              </div>
            </article>
          ))}
