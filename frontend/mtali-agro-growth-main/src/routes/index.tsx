  ...
  head: () => ({
    meta: [
      { title: "Mtali Agro Traders — Crop Nutrition for Tanzania's Growers" },
      {
        name: "description",
        content:
          "Premium foliar and water-soluble fertilizers from Arusha. Trusted by farmers, cooperatives and dealers across Tanzania.",
      },
      { property: "og:title", content: "Mtali Agro Traders — Crop nutrition for Tanzania" },
      {
        property: "og:description",
        content:
          "Premium foliar and water-soluble fertilizers, engineered in Arusha for Tanzanian soils.",
      },
    ],
  }),
  component: HomePage,
  ...
  return (
    <section className="relative overflow-hidden bg-ink text-primary-foreground">
      ...
      <div className="absolute inset-0">
        <img
          src={heroImg}
          alt="Premium Victory fertilizers in a Tanzanian field"
            ...
            <div className="mt-10 grid gap-8 md:grid-cols-12 md:items-end">
              ...
              <div className="md:col-span-5 flex flex-wrap items-center gap-5">
                ...
                <a
                  ...
                  className="link-arrow text-primary-foreground hover:text-accent"
                >
      ...
      <div className="relative border-t border-primary-foreground/15 bg-ink/70 backdrop-blur">
        <div className="container-pad">
          <div className="bg-ink px-5 py-7 md:px-7 md:py-9">
            ...
            <div className="mt-2 font-mono text-[10px] uppercase tracking-[0.18em] text-primary-foreground/60">
              {t("stat.cert")}
            </div>
          </div>
        </div>
      ...
      <div className="flex gap-12 whitespace-nowrap font-mono text-[11px] uppercase tracking-[0.25em] text-muted-foreground animate-[scroll_40s_linear_infinite]">
        {[...items, ...items, ...items].map((x, i) => (
          <span key={i} className="flex items-center gap-12">
            {x} <span className="text-primary/40">✦</span>
          </span>
        ))}
      </div>
    ...
    <section className="section-pad">
      <div className="container-pad grid gap-16 md:grid-cols-12 md:gap-10">
        ...
        <div className="md:col-span-8">
          <p className="display text-3xl text-ink md:text-5xl">{t("manifesto.body")}</p>
  ...
  return (
    <section className="section-pad">
      <div className="container-pad text-center">
        ...
        <div className="mt-10">
          <Link to="/products" className="btn-primary">
            {t("cta.allProducts")} <ArrowUpRight className="h-4 w-4" />
          </Link>
        </div>
      </div>
    </section>
            ...
            latestPosts.map((post) => (
              ...
              >
                <div className="aspect-16/10 overflow-hidden bg-muted">
                  <img
                    src={post.cover_image_url || maize}
                    alt={post.title_en}
                ...
                <div className="p-6">
                  <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
                    {post.category || t("category.general")}
                  </span>
                  <h3 className="mt-3 font-display text-xl font-bold text-foreground group-hover:text-primary">
                    {lang === "en"
                      ? sanitizePlainText(post.title_en)
                      : sanitizePlainText(post.title_sw || post.title_en)}
                  </h3>
                  <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
                    {lang === "en"
                      ? truncatePlainText(post.excerpt_en || post.body_en, 120)
                      : truncatePlainText(post.excerpt_sw || post.body_sw || post.body_en, 120)}
                  </p>
                  <div className="mt-6 flex items-center justify-between gap-3">
                    <Link to="/blog" className="text-sm font-semibold text-primary">
                      {t("blog.read")} <ArrowUpRight className="inline-block h-4 w-4" />
                    </Link>
                  </div>
                </div>
              </article>
            ...
            <div className="rounded-3xl border border-dashed border-border bg-secondary/30 p-12 text-center">
              ...
              <div className="mt-6">
                <Link to="/blog" className="btn-primary">
                  {t("cta.allNotes")} <ArrowUpRight className="h-4 w-4" />
                </Link>
              </div>
            </div>
          )}
...
function Contact() {
  ...
  const items = [
    {
      l: t("contactSec.hq"),
      v: getSettingValue(settings, "contact.hqAddr", t("contactSec.hqAddr")),
    },
    { l: t("contactSec.phone"), v: `${contactPhone}\n${contactPhoneAlt}` },
    { l: t("contactSec.email"), v: getSettingValue(settings, "contact.email", CONTACT_EMAIL) },
    {
      l: t("contactSec.hours"),
      v: getSettingValue(settings, "contact.hours", t("contactSec.hoursVal")),
    },
  ];
        ...
        <div className="md:col-span-7">
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            <Link to="/contact" className="btn-primary">
              {t("cta.inquire")} <ArrowUpRight className="h-4 w-4" />
            </Link>
            <a
              href={waLink(undefined, `Habari Mtali Agro, ningependa habari zaidi.`)}
              target="_blank"
              rel="noopener noreferrer"
              className="link-arrow text-primary-foreground hover:text-accent"
            >
            ...
            <a
              href={waLink(WHATSAPP_NUMBER_2, `Habari Mtali Agro, ningependa habari zaidi.`)}
              target="_blank"
              rel="noopener noreferrer"
              className="link-arrow text-primary-foreground hover:text-accent"
            >
